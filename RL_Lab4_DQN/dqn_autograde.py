...
def get_epsilon(it):
    # YOUR CODE HERE
    epsilon = np.max([1 - (it / 1000.0), 0.05])
    return epsilon
...
def compute_q_vals(Q, states, actions):
    ...
    """
    # YOUR CODE HERE
    qnet_input = torch.reshape(states, (states.shape[0], 4)).float()
    q_vals = Q(qnet_input)
    output = torch.gather(q_vals, 0, actions)
    print(states)
    print(q_vals)
    print(output)
    print('---------------')
...
def compute_targets(Q, rewards, next_states, dones, discount_factor):
    ...
    if dones.sum() > 0:
        ...
        return rewards + discount_factor * q_vals
